...
sends these arguments as a stringified JSON payload which your ship's backend
was able to parse and then send the right contents to the right Hoon program for
execution.
...
> code block of paylaod sent

In this case, our `data` value was just the string 'hi. :)'. Our `params`
object contained two name-value pairs: our `appl`, for 'application', was
`hood`, your ship's system daemon, and our `mark`, or message type, was a
`helm-hi`, which just told `hood` to run our simple *helm-hi* Hoon program to
take the data value sent and print it to the screen, like a traditional 'Hello,
world!'. And upon the successful request, our callback function simply
console.logged the request response; our ship sent us an `{ok:true}`, meaning
"everything is all good!". And it console.logged a hard-coded string, telling us
we just said hi to our ship.
...
Now let's do the same thing, but this time from your Talk web UI. To do this, as said above, we first need to `Log In` to authenticate your ship through Tree.
...
For now, we store your web password in your ship. You can retrieve it with the `+code` Dojo command.
...
We know from the previous section that regular in-HTML `<script>` JavaScript works for trivial functions. For more complex functionality, of course, we'll want to separate our frontend code into its own directory and simply source it from our HTML `<head>` like we did with urb.js.

Thankfully, we've organized this for you in the `1.3` branch, so let's `checkout` once again and `cp` the code:
